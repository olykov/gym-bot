...
export default async function handler(req: NextApiRequest, res: NextApiResponse<MuscleSetsWeeklyResponse | { error: string }>) {
  ...
  try {
    ...
    // Fetch weekly muscle sets data
    const weeklyData = await getWeeklyMuscleSets(username);
    
    res.status(200).json(weeklyData);
  } catch (error) {
    console.error('Error fetching weekly muscle sets:', error);
...
async function getWeeklyMuscleSets(username: string): Promise<MuscleSetsWeeklyResponse> {
  ...
  try {
    // Get weekly sets data by muscle for last 12 months
    const weeklyQuery = `
      WITH weekly_data AS (
        SELECT 
    ...
    console.log('🔍 Raw data from query:', result.rows.slice(0, 3)); // Debug first 3 rows
    ...
    console.log('📈 Series data:', series.map(s => ({ name: s.name, dataPoints: s.data.length })));
    ...
    console.error('❌ WeeklyMuscleSets: Database error:', error);
    throw error;
  } finally {
    client.release();
  }
